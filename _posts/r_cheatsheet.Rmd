...
# code_example
```
I welcome you to leave comments or propose changes to the document on github,
...
Performs a two-sample `z`-test, testing for the difference in two means when the
variance of both groups is known. Returns the estimate of the group means, the
test statistic, a $p$-value, and a confidence interval (whose level is specified
by the `conf.level` parameter).
...
Performs a two-sample `t`-test, testing for the difference in two means when the
variance of both groups are *unknown*. We typically assume equal variance
(`var.equal = TRUE`) unless an initial / diagnostic analysis explicitly suggests
to do otherwise. Returns the estimate of the group means, the test statistic, a
